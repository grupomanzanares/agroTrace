...
export class ResetPasswordPage implements OnInit {
  ...
  ngOnInit() {
          ...
          this._authService.validateToken(this.token).subscribe({
            ...
            error: (error) => {
              this._toastService.presentToast( 'El enlace de recuperación no es válido o ha expirado.', 'danger', 'top');
  ...
  async resetPassword() {
    ...
    if (password !== confirmPassword) {
      this._toastService.presentToast( 'Las contraseñas no coinciden.', 'danger', 'top');
      return;
    }
    ...
    this._authService.resetPassword(this.token, password).subscribe({
      ...
      error: async (error) => {
        await this._loadingService.hideLoading();
        this._toastService.presentToast( 'Error al restablecer la contraseña. Intente nuevamente.', 'danger', 'top');
